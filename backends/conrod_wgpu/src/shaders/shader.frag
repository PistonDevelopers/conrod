...
layout(set = 0, binding = 0) uniform texture2D texture_Color;
layout(set = 0, binding = 1) uniform sampler sampler_Color;
...
void main() {
    // Text
    if (v_Mode == uint(0)) {
        float a = texture(sampler2D(texture_Color, sampler_Color), v_Uv).r;
        Target0 = v_Color * vec4(1.0, 1.0, 1.0, a);

    // Image
    } else if (v_Mode == uint(1)) {
        Target0 = texture(sampler2D(texture_Color, sampler_Color), v_Uv);
