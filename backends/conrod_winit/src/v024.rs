...
#[macro_export]
macro_rules! v024_convert_mouse_button {
    ($mouse_button:expr) => {{
...
#[macro_export]
macro_rules! v024_convert_window_event {
    ($event:expr, $window:expr) => {{
        ...
        let map_key = |key: winit::event::VirtualKeyCode| $crate::v024_convert_key!(key);
        let map_mouse = |button: winit::event::MouseButton| $crate::v024_convert_mouse_button!(button);
...
#[macro_export]
macro_rules! v024_convert_event {
    ($event:expr, $window:expr) => {{
        match $event {
            winit::event::Event::WindowEvent { event, .. } => {
                $crate::v024_convert_window_event!(event, $window)
            }
            _ => None,
        }
    }};
...
/// Requires that both the `conrod_core` and `winit` crates are in the crate root.
#[macro_export]
macro_rules! v024_convert_mouse_cursor {
    ($cursor:expr) => {{
...
#[macro_export]
macro_rules! v024_conversion_fns {
    () => {
        ...
        pub fn convert_key(
            ...
            $crate::v024_convert_key!(keycode)
        }
        ...
        pub fn convert_mouse_button(
            ...
            $crate::v024_convert_mouse_button!(mouse_button)
        }
        ...
        pub fn convert_mouse_cursor(
            ...
            $crate::v024_convert_mouse_cursor!(cursor)
        }
        ...
        pub fn convert_event<T>(
            ...
            $crate::v024_convert_event!(event, window)
        }
    };
}
