...
#[cfg(feature="conrod_winit_glue")] mod support;
...
mod feature {
    extern crate conrod_gfx_backend;
    ...
    use gfx_core;
    use support;
    use self::conrod_winit_backend::winit;
    ...
    pub fn main() {
        ...
        let (window, mut device, mut factory, rtv, _) =
            gfx_window_glutin::init::<conrod_gfx_backend::ColorFormat, DepthFormat>(builder, context, &events_loop );
        let mut encoder: gfx::Encoder<_, _> = factory.create_command_buffer().into();

        let mut renderer = conrod_gfx_backend::Renderer::new(&mut factory, &rtv, window.get_hidpi_factor() as f64).unwrap();
        ...
        let rust_logo = image_map.insert(load_rust_logo::<conrod_gfx_backend::ColorFormat,_,_>(&mut factory));
        ...
        'main: loop {
            ...
            events_loop.poll_events(|event|{
                ...
                if let Some(event) = conrod_winit_backend::convert_event(event.clone(), window.window()) {
                    ...
                    winit::Event::WindowEvent{event, .. } =>
                        match event {
                            ...
                            winit::WindowEvent::Resized(logical_size) => {
                                ...
                                let (new_color, _) = gfx_window_glutin::new_views::<conrod_gfx_backend::ColorFormat, DepthFormat>(&window);
...
mod feature {
    pub fn main() {
        println!("This example requires the `conrod_winit_glue` feature. \
                 Try running `cargo run --release --no-default-features --features=\"conrod_winit_glue\" --example <example_name>`");
   }
}
