...
use input::{self, Button, MouseButton, Provider};
...
fn mouse_button_down_should_return_none_if_mouse_is_not_over_widget() {
    ...
    let widget_input = input::Widget::for_widget(Index::Public(Id(2)), widget_area, &global_input);
...
fn mouse_clicks_should_be_relative_to_widget_position() {
    ...
    let widget_input = input::Widget::for_widget(idx, rect, &global_input);
